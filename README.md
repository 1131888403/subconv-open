...
- 支持生成 Clash/Mihomo、sing-box、V2Ray、Surge、Quantumult X 配置；Clash 默认使用扁平的 `🚀 节点选择` 手动选择组，所有节点直接列入，不按国家拆分标签；可选 `♻️ 自动选择` 测速组；
...
1. 输出格式选择 **Clash Meta (Mihomo)**。
2. 解析完成后，点击节点右侧的删除按钮，或使用关键词、`type:`、`port:`、`re//` 批量删减。
3. 确认页面摘要中的“生效节点”数量正确，然后点击 **导出配置**。
4. 将下载的 YAML 文件导入 Clash Meta、Mihomo 或 Clash Verge。
5. 在客户端进入 `🚀 节点选择`，即可直接手动选择删减后保留的任意节点。
6. 如果希望自动选择延迟较低的节点，可选择独立的 `♻️ 自动选择` 组；它不会影响手动选择组。

网页生成的 Clash 配置不会按国家生成美国、德国、新加坡等嵌套小组，以避免部分 Android 客户端把国家组显示成无法展开或无法选择的标签。
